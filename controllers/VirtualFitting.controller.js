...
export const tryOnProduct = async (req, res) => {
  try {
    ...
    if (productId) {
      ...
      const tryOnResult = await generateVirtualTryOn(virtualFitting.bodyModel, customDesignModel)
...
export const deleteTryOnEntry = async (req, res) => {
  try {
    ...
    if (entryToDelete.tryOnModelUrl) {
      const modelPath = path.join(__dirname, "../public", entryToDelete.tryOnModelUrl)
    ...
    if (entryToDelete.fittingImage && entryToDelete.fittingImage.includes("cloudinary")) {
      const publicId = entryToDelete.fittingImage.split("/").slice(-1)[0].split(".")[0]
      await cloudinary.uploader.destroy(`virtual-fitting/${req.user._id}/tryons/${publicId}`)
    }
...
function calculateFittingResult(virtualFitting, product, size) {
  ...
  const sizeMeasurements = sizeIndex >= 0 ? product.sizes[sizeIndex].measurements || {} : {}

  // Combine product measurements with size-specific adjustments
  const finalProductMeasurements = { ...productMeasurements, ...sizeMeasurements }

  // Calculate fit scores for different body parts
  const fitScores = {
    shoulders: calculateFitScore(userMeasurements.shoulders, finalProductMeasurements.shoulders),
    bust: calculateFitScore(userMeasurements.bust, finalProductMeasurements.bust),
    waist: calculateFitScore(userMeasurements.waist, finalProductMeasurements.waist),
    hips: calculateFitScore(userMeasurements.hips, finalProductMeasurements.hips),
    length: calculateFitScore(userMeasurements.height, finalProductMeasurements.length),
  }
  ...
  const finalScore = totalWeight > 0 ? Math.round(overallScore / totalWeight) : 85
...
function calculateFitScore(userMeasurement, productMeasurement) {
  ...
  const difference = Math.abs(userMeasurement - productMeasurement) / productMeasurement
